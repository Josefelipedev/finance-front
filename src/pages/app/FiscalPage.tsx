...
const TAG_CLASSES: Record<FiscalTag, string> = {
  mensal: 'bg-sky-50 text-sky-700 dark:bg-sky-400/10 dark:text-sky-300',
        ...
        actions={
          isEditing ? (
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50 dark:border-white/[0.08] dark:text-slate-300 dark:hover:bg-white/5"
            >
      ...
      >
        <Surface className="p-6 sm:p-8">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-500 dark:text-slate-400">
            Este painel é para <strong className="font-medium text-slate-700 dark:text-slate-200">trabalhadores
          ...
          <form onSubmit={handleSubmit} className="mt-6 grid max-w-2xl grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <select
                value="PT"
                disabled
                className="w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 dark:border-white/[0.08] dark:bg-slate-700/60 dark:text-white"
              >
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                className="w-full max-w-xs rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-slate-900 placeholder:text-slate-400 focus:border-brand-400 focus:outline-none dark:border-white/[0.08] dark:bg-slate-700 dark:text-white"
              />
            </div>
            <div className="sm:col-span-2">
              <button
    ...
    >
      ...
      <Surface className="p-5 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Portugal · {profile.regimeLabel}</p>
            </div>
          </div>
          {profile.fiscalNumber && (
            <div className="text-left sm:text-right">
              ...
              <p className="font-display text-lg font-semibold tabular-nums text-slate-900 dark:text-white">
                {profile.fiscalNumber}
              </p>
            </div>
          )}
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <span className="inline-flex items-center gap-2 rounded-full bg-sky-50 px-3 py-1.5 text-xs font-medium text-sky-700 dark:bg-sky-400/10 dark:text-sky-300">
            <span className="h-2 w-2 rounded-full bg-sky-500"></span>
            {profile.ivaLabel}
          </span>
          <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 dark:border-white/[0.08] dark:text-slate-300">
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-slate-200 p-3 dark:border-white/[0.06]">
            <p className="text-[11px] font-medium uppercase tracking-wider text-slate-400 dark:text-slate-500">
              Início de atividade
            </p>
      ...
      {next && nextCal && (
        <Surface className="overflow-hidden">
          <div className="flex items-center gap-4 p-5 dark:bg-gradient-to-br dark:from-brand-400/[0.10] dark:to-transparent sm:p-6">
            <div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white dark:border-white/[0.08] dark:bg-slate-900">
              <span className="text-[11px] font-bold uppercase tracking-wider text-rose-500">
                {nextCal.m}
              </span>
              <span className="font-display text-2xl font-semibold leading-none text-slate-900 dark:text-white">
                {nextCal.d}
              </span>
            </div>
            <div className="min-w-0">
              ...
              <p className="mt-0.5 font-medium text-slate-900 dark:text-white">{next.title}</p>
              <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">{next.description}</p>
            </div>
          </div>
        </Surface>
      )}
      ...
      {upcoming.length > 0 && (
        <div>
          ...
          <Surface className="divide-y divide-slate-100 dark:divide-white/[0.06]">
            {upcoming.map((item, i) => (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {obligations.map((ob) => (
              <Surface key={ob.key} className="flex flex-col gap-2 p-5">
                ...
                <h3 className="font-medium text-slate-900 dark:text-white">{ob.title}</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400">{ob.description}</p>
                {ob.meta && (
      ...
      <div>
        ...
        <Surface className="p-5 sm:p-6">
          ...
          <div className="mt-4 h-3 overflow-hidden rounded-full border border-slate-200 bg-slate-100 dark:border-white/[0.06] dark:bg-slate-900">
            <span className="block h-full w-0 rounded-full bg-gradient-to-r from-brand-400 to-sky-400"></span>
          </div>
          <div className="mt-2 flex justify-between text-xs tabular-nums text-slate-400 dark:text-slate-500">
            <span>0 €</span>
          ...
          <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">
            ...
            ultrapassar <strong className="font-medium text-slate-700 dark:text-slate-200">{formatEur(profile.thresholdEur)}</strong>,
            entrega declaração de alterações e passa ao regime normal de IVA. Se ultrapassar em mais
            de 25% (<strong className="font-medium text-slate-700 dark:text-slate-200">{formatEur(profile.thresholdEur * 1.25)}</strong>),
            o prazo é de <strong className="font-medium text-slate-700 dark:text-slate-200">15 dias úteis</strong>; caso
            contrário, em <strong className="font-medium text-slate-700 dark:text-slate-200">janeiro do ano seguinte</strong>.
          </p>
        </Surface>
      </div>
      ...
      {/* 6. Nota / disclaimer */}
      {disclaimer && (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-xs leading-relaxed text-slate-500 dark:border-white/[0.10] dark:bg-white/[0.02] dark:text-slate-400">
          <strong className="font-medium text-slate-700 dark:text-slate-300">Nota.</strong> {disclaimer}
        </div>
      )}
    </PageShell>
  );
