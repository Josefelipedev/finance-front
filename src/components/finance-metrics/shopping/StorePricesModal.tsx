...
const StorePricesModal: React.FC<StorePricesModalProps> = ({
  itemName,
  prices,
  isLoading,
  onClose,
    ...
    <Modal isOpen={true} onClose={onClose} className="max-w-md">
      <div className="p-5">
        <div className="mb-4 pr-8">
          <h3 className="text-base font-semibold text-gray-800 dark:text-white">Comparar Preços</h3>
            ...
            {sorted.map((p, idx) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">{STORE_EMOJI[p.supermarket] ?? '🏷️'}</span>
                  <div>
                    <p
                      className={`text-sm font-medium ${idx === 0 ? 'text-green-700 dark:text-green-400' : 'text-gray-700 dark:text-gray-200'}`}
                    >
                ...
                <span
                  className={`text-sm font-bold ${idx === 0 ? 'text-green-700 dark:text-green-400' : 'text-gray-700 dark:text-gray-200'}`}
                >
                  €{p.price.toFixed(2)}
                </span>
              </div>
