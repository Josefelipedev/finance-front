...
const RecurringForm: React.FC<RecurringFormProps> = ({ transaction, onSuccess, onCancel }) => {
  ...
  return (
    <Modal
      isOpen
      onClose={onCancel}
      className="mx-4 max-h-[90dvh] max-w-md"
      bodyClassName="flex max-h-[90dvh] flex-col"
    >
      {/* Cabeçalho fixo: com o formulário a crescer, o título saía do ecrã. */}
      <div className="shrink-0 border-b border-gray-200 px-6 py-4 pr-16 dark:border-white/[0.06]">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white">
          {transaction ? 'Editar Transação Recorrente' : 'Nova Transação Recorrente'}
        </h3>
        <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
          {transaction?.description || 'Gera contas a pagar ou a receber automaticamente'}
        </p>
      </div>

      <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
        {/* Só o miolo rola. O `data-modal-scroll` faz os calendários fecharem-se
            ao rolar, em vez de ficarem a pairar longe do campo. */}
        <div data-modal-scroll className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-5">
          {/* Descrição */}
          <div>
        ...
        {/* Rodapé fixo: para guardar era preciso rolar até ao fim. */}
        <div className="shrink-0 border-t border-gray-200 px-6 py-4 dark:border-white/[0.06]">
          {submitError && (
            <div className="mb-3 rounded-lg border border-error-500 bg-error-50 px-3 py-2 dark:bg-error-500/10">
