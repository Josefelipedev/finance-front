...
export default function SignUpForm() {
  ...
  const fillFormWithGoogleData = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (userInfo.given_name) setValue('firstName', userInfo.given_name, { shouldValidate: true });
        if (userInfo.family_name) setValue('lastName', userInfo.family_name, { shouldValidate: true });
        if (userInfo.email) setValue('email', userInfo.email, { shouldValidate: true });
        setValue('agreeToTerms', true, { shouldValidate: true });
        await trigger(['firstName', 'lastName', 'email', 'agreeToTerms']);
      } catch {
    ...
    onError: () => {
      setGoogleError('Falha na conexão com o Google.');
      setIsGoogleLoading(false);
    },
    scope: 'email profile',
  });
            ...
            <form onSubmit={handleSubmit(onSubmit)} noValidate>
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <Input type="tel" id="phone" placeholder="+55 (11) 98765-4321" {...register('phone')} />
                  <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">
                    Add your WhatsApp number to use the AI financial assistant.
                  </p>
                ...
                {errors.agreeToTerms && <p className="text-sm text-error-500">{errors.agreeToTerms.message}</p>}
